...
if ($search && !$error) {
    ...
    $search_db = '%' . $search_db . '%';
    $total = $db->query("SELECT COUNT(*) FROM `users` WHERE `nickname` LIKE " . $db->quote($search_db))->fetchColumn();
    ...
    if ($total) {
        $req = $db->query("SELECT * FROM `users` WHERE `nickname` LIKE " . $db->quote($search_db) . " ORDER BY `name` ASC" . $tools->getPgStart(true));
        $i = 0;
        while ($res = $req->fetch()) {
