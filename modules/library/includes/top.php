...
ORDER BY `avg` DESC, `cnt` DESC LIMIT " . $start . ',' . $userConfig->kmess);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $uploader = $row['uploader_id'] ? '<a href="' . $container->get('config')['setting']['homeurl'] . '/profile/?user=' . $row['uploader_id'] . '">' . $tools->checkout($row['uploader']) . '</a>' : $tools->checkout($row['uploader']);
