        ...
        foreach (new LimitIterator(new ArrayIterator($stats), $start, $userConfig->kmess) as $txt) {
            ...
            if ($row) {
                ...
                echo '<div class="list' . (++$i % 2 ? 2 : 1) . '">'
                    ...
                    . '<div class="sub">' . _t('Who added') . ': ' . '<a href="' . App::getContainer()->get('config')['setting']['homeurl'] . '/profile/?user=' . $row['uploader_id'] . '">' . $tools->checkout($row['uploader']) . '</a>' . ' (' . $tools->displayDate($row['time']) . ')</div>'
